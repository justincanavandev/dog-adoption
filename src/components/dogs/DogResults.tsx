import { type MutableRefObject, useContext, useRef, useCallback } from "react";
...
import type { DogData, DogWithRelations } from "~/types/dog-types";
...
const DogResults = () => {
  ...
  const paginatedDogs = newDogData?.pages[currentPage]?.dogs;
  const totalDogs = newDogData?.pages[currentPage]?.totalDogs;
  const totalPages = totalDogs ? Math.ceil(totalDogs / 10) : "";
  ...
  const handleFetchNextPage = useCallback(async () => {
    try {
      ...
      console.error("Unable to fetch next page", e);
    }
  }, [fetchNextPage, setCurrentPage, setDogData]);
